{-# LANGUAGE GADTs, RankNTypes, ScopedTypeVariables, DeriveDataTypeable, DeriveGeneric, StandaloneDeriving, ViewPatterns #-}
...
instance Num UInt128 where
  -- The errors got too tedious to track down. Making it modulo.
  ...
  --fromInteger n_ =
  --  let n = n_ `mod` 0x100000000000000000000000000000000 in
  fromInteger n =
    case n `divMod` 0x10000000000000000 of
      (hi, lo) -> UInt128 (fromInteger hi) (fromInteger lo)
...
instance Show UInt128 where
  ...
  show (toInteger -> i) =
    if i < 0x100000000 then printf "0x%x" i else printf "0x%032x" i
